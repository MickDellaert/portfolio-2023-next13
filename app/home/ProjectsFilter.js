      ...
      <div className="relative z-50 flex items-center justify-start ">
        <div className="">
          {uniqueTechNames.map((techName) => (
            <button
              className={`mr-2 mb-2 h-6 rounded-lg py-px px-2 text-[0.60rem] font-semibold leading-none 
              sm:mr-3 sm:py-1 sm:px-2 sm:text-xs md:hover:bg-primary md:hover:text-white 
              ${
                activeTags?.includes(techName)
                  ? "bg-primary text-white" : "bg-neutral-300"
              }`}
          ...
          {filters.length > 0 && (
            <button
              onClick={() => setFilters([])}
              className="mr-3 mb-2 h-6 rounded-lg border-2 border-primary py-1 px-2 text-xs font-semibold leading-none 
